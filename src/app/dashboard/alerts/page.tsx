...
export default async function AlertsPage() {
  ...
  const severityStyles: Record<string, { dot: string; bg: string; text: string }> = {
    ...
    info: { dot: "bg-gray-400", bg: "bg-surface-secondary", text: "text-text" },
  };
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border-light">
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Severity</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Alert</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Meter</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Status</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Triggered</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border-light">
                {alerts.map((alert: any) => {
                  const sev = severityStyles[alert.severity] ?? severityStyles.info;
                  const statClass = statusStyles[alert.status] ?? "bg-surface-secondary text-text";
                  return (
                    <tr key={alert.id} className="hover:bg-surface-hover transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="font-medium text-text text-sm">{alert.title}</div>
                        {alert.message && (
                          <div className="text-xs text-text-muted mt-0.5 max-w-xs truncate">{alert.message}</div>
                        )}
                      </td>
