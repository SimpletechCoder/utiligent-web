...
export default function AuditLogClient() {
  ...
  async function fetchAuditLogs() {
    ...
    // Access control: the parent server component (page.tsx) enforces the
    // `audit.view` permission before this component is rendered. Row scoping to
    // the caller's organization is enforced by RLS on `audit_logs`.
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-text-muted mt-1">Track all system changes and user actions across your organization</p>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-border p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div>
            <label className="text-sm font-medium text-text block mb-2">Action (Search)</label>
            <input
              type="text"
              value={filterAction}
          ...
          <div>
            <label className="text-sm font-medium text-text block mb-2">Start Date</label>
            <input
              type="date"
          ...
          <div>
            <label className="text-sm font-medium text-text block mb-2">End Date</label>
            <input
              type="date"
              value={filterEndDate}
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full px-4 py-2 text-sm font-medium text-text-secondary bg-surface-secondary border border-border rounded-lg hover:bg-surface-hover transition-colors"
            >
              Reset
            </button>
          </div>
      ...
      {loading ? (
        <div className="bg-surface rounded-xl border border-border px-6 py-12 text-center">
          <div className="inline-block">
            <div className="animate-spin w-8 h-8 border-3 border-border border-t-brand rounded-full" />
          </div>
          <p className="text-text-muted mt-4">Loading audit logs...</p>
        </div>
        ...
        <div className="bg-surface rounded-xl border border-border px-6 py-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-text mb-1">No audit logs</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border-light">
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Timestamp</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">User</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Action</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Entity Type</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Entity ID</th>
                  <th className="text-left text-xs font-medium text-text-secondary uppercase tracking-wider px-6 py-3">Details</th>
                </tr>
              </thead>
                  ...
                  <tr key={log.id} className="hover:bg-surface-hover transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <button
                        ...
                        className="px-2.5 py-1 rounded text-xs font-medium bg-surface-secondary text-text-secondary hover:bg-surface-hover transition-colors"
                      >
      ...
      {/* Info note */}
      <div className="mt-6 bg-brand-light border border-brand rounded-xl px-6 py-4">
